package com.tata.UserService.aspect;
...
public class LoggingAspect {
	...
	@Before("execution(* com.tata.UserService.*.*.*(..)) ")
	public void logBeforeMethodExecution(JoinPoint joinPoint) {
		Signature signature = joinPoint.getSignature();
		String className = signature.getDeclaringType().getSimpleName();
		String methodName = signature.getName();
		String methodArgs = Arrays.toString(joinPoint.getArgs());
		logger.info(className + " :: " + methodName + " :: start :: with args - " + methodArgs);
	}

	@After("execution(* com.tata.UserService.*.*.*(..)) ")
	...
	@AfterThrowing(pointcut = "execution(* com.tata.UserService.*.*.*(..))", throwing = "ex")
	...
	@AfterReturning(pointcut = "execution(* com.tata.UserService.*.*.*(..))", returning = "returnValue")
	public void logAfterReturnedValue(JoinPoint joinPoint, Object returnValue) {
		...
		logger.info("Returned from " + className + "." + methodName + ": " + returnValue);
	}
	...
	private boolean hasRepositoryAnnotation(Class<?> clazz) {
		...
		for (Annotation annotation : annotations) {
			if (annotation.annotationType().getSimpleName().equals("Repository")) {
				return true;
			}
		}
		return false;
